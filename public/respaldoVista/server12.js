...
const session = require('express-session'); // <-- Debe estar presente
...
app.use(session({
    ...
    cookie: {
        ...
        path: '/' // --- CLAVE: La cookie es válida para todo el sitio ---
    }
}));
...
// Conexión a la Base de Datos MySQL (para consultas simples y callbacks antiguos)
const db = mysql.createConnection({
    host: 'localhost',
    user: 'root',
    password: '',
...
// --- SOLUCIÓN: Crear el Pool de conexiones para las rutas async/await ---
// Esto es necesario para que pool.query funcione en las rutas de correcciones
const pool = mysql.createPool({
    ...
    connectionLimit: 10, // Puedes ajustar esto según tus necesidades
    queueLimit: 0
});
*/

// Configuración de la Sesión (Ya la tienes, esto es referencia)
app.use(session({
...
// Usaremos un Pool de conexiones para soportar consultas simples (db.query)
// Y consultas modernas async/await (pool.query).
//const mysql = require('mysql2'); // Asegúrate de tener esta línea arriba
...
// Conectar para verificar (usando el pool)
pool.getConnection((err, connection) => {
    if (err) {
        console.error('❌ Error al conectar a la base de datos MySQL:', err.message);
        return;
    }
    console.log('✅ Conectado exitosamente al Pool de conexiones MySQL (asesores).');
    connection.release(); // Liberar conexión de prueba
});

// Exportar el pool como 'db' para que el resto de tu código (que usa db.query) funcione
const db = pool;
// -----------------------------------------------
db.connect((err) => {
    if (err) {
...
app.post('/api/login', (req, res) => {
    ...
    db.query(sql, [usuario, clave], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            // --- CLAVE: Guardar explícitamente la sesión en la base de datos/memoria ---
            req.session.save((err) => {
                ...
                // --- CLAVE: Solo responder al frontend DESPUÉS de guardar ---
                console.log('✅ Sesión guardada correctamente para:', user.usuario);
...
app.get('/api/tipo_asesoria', (req, res) => {
    // Si tu tabla se llama tipo_asesoria o similar:
...
// Ruta para servir la vista de correcciones
app.get('/correcciones', (req, res) => {
    res.sendFile(__dirname + '/views/correcciones.html'); // Ajusta la ruta de la carpeta si es necesario
});


// Ruta para obtener todos los registros de control de correcciones
app.get('/api/control_correcciones', async(req, res) => {
    try {
        const [rows] = await pool.query('SELECT * FROM control_correcciones ORDER BY fecha DESC');
        res.json({ success: true, data: rows });
    } catch (error) {
...
app.post('/api/control_correcciones', async(req, res) => {
    try {
        ...
        res.json({ success: true, message: 'Corrección registrada con éxito', id: result.insertId });
    } catch (error) {
