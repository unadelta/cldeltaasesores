...
app.get('/api/materias', async(req, res) => {
    try {
        ...
        // --- CORRECCIÓN ---
        // Esta línea evita la expulsión al login.
        // Responde con un error 500 al cliente (navegador).
        res.status(500).json({
            success: false,
            message: 'Error en el servidor al consultar materias. Verifique la consola del servidor para detalles.'
        });
    }
});
...
app.get('/api/materias', async(req, res) => {
    try {
        ...
        // --- CORRECCIÓN AQUÍ ---
        // Esto es lo que te expulsa al login. Debes cambiarlo.
        // return res.redirect('/'); // <--- ELIMINA ESTA LÍNEA

        // --- USA ESTA LÍNEA EN SU LUGAR ---
        // Responde con un error 500 al cliente. El usuario se queda en la página.
        res.status(500).json({ success: false, message: 'Error en el servidor al consultar materias. Verifique la consola.' });
    }
});
...
app.get('/api/materias', async(req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: ' Error en el servidor al consultar las materias' });
    }
});
