...
// --- CONFIGURACIÓN ACTUAL (LA QUE ESTÁ FALLANDO) ---
/*
app.use(session({
    secret: 'clave_secreta_asesorias_una',
    resave: true,           // <--- Recomendado cambiar a false
    saveUninitialized: true, // <--- Recomendado cambiar a false
    cookie: {
        secure: false,      // Funciona en HTTP local
        httpOnly: true,
...
*/
/*
// --- NUEVA CONFIGURACIÓN CORREGIDA (PARA ENTORNO LOCAL) ---
app.use(session({
    secret: 'clave_secreta_asesorias_una',
    resave: false, // Evita guardar la sesión si no hubo cambios
    saveUninitialized: false, // No crear sesión hasta que se guarden datos (ahorra espacio)
    cookie: {
        secure: false, // IMPORTANTE: Mantener en false para HTTP local
        httpOnly: true, // Evita que JS del cliente lea la cookie (seguridad)
        maxAge: 24 * 60 * 60 * 1000,
        sameSite: 'lax' // IMPORTANTE: Relaja la restricción de envío de cookie en redirecciones
    }
}));
*/
/*
// server.js - Configuración de Sesión (PROBAR ESTO)
app.use(session({
    secret: 'clave_secreta_asesorias_una',
    resave: false,
    saveUninitialized: false,
    cookie: {
        secure: false, // ¡Muy importante que sea false en local!
        httpOnly: false, // Permite que JS acceda (solo para depurar, luego poner a true)
        maxAge: 24 * 60 * 60 * 1000,
        sameSite: 'none', // ¡Relaja al máximo la seguridad! (Requiere secure: false en HTTP o true en HTTPS)
        // NOTA: sameSite: 'none' requiere que 'secure' sea 'true' en Chrome moderno,
        // pero 'secure: true' solo funciona si usas HTTPS.
        // Si usas HTTP en local, usa sameSite: 'lax' o 'strict'
    }
}));
...
/*
// ==========================================
// CONFIGURACIÓN DE SESIÓN (SOLUCIÓN PARA ENTORNO LOCAL - HTTP)
// ==========================================
app.use(session({
    secret: 'clave_secreta_asesorias_una', // Mantén tu clave secreta
    resave: false,
    saveUninitialized: false,
    name: 'asesorias.sid', // Le damos un nombre personalizado a la cookie (opcional, pero ayuda)
    cookie: {
        secure: false, // ¡CRUCIAL! Debe ser false en local (HTTP). Si es true, no funcionará.
        httpOnly: true, // Mantiene la seguridad (no accesible por JS del cliente)
        maxAge: 24 * 60 * 60 * 1000, // 1 día
        // --- CONFIGURACIÓN CLAVE PARA EVITAR BLOQUEO EN REDIRECCIONES ---
        // Usamos 'lax' para permitir el envío de la cookie al navegar desde el login a otras páginas.
        sameSite: 'lax'
    }
}));
*/
...
// ==========================================
// CONFIGURACIÓN DE SESIÓN (ÚLTIMO INTENTO - MÁXIMA PERMISIVIDAD LOCAL)
// ==========================================
...
app.use(session({
    ...
    resave: false,
    saveUninitialized: false,
    name: 'asesorias.sid', // Nombre personalizado
    cookie: {
        secure: false, // ¡CRUCIAL! Debe ser false en HTTP local.
        httpOnly: true, // Mantiene la seguridad.
        maxAge: 24 * 60 * 60 * 1000, // 1 día.
        // Usamos 'none' para indicar que la cookie puede enviarse en peticiones de terceros.
        // NOTA: Para que 'none' funcione en Chrome moderno, 'secure' DEBE ser 'true'.
        // PERO 'secure: true' solo funciona en HTTPS.
        // --- SI ESTO NO FUNCIONA, EL PROBLEMA ES QUE NECESITAS HTTPS ---
        // Como alternativa local, prueba cambiando 'none' por 'lax' nuevamente.
        sameSite: 'none',
        secure: true // <--- ¡PRUEBA ESTO SOLO SI USAS HTTPS EN LOCAL! Si no, ponlo en false.
    }
}));
...
// Ruta para obtener el listado completo de materias, objetivos y calificaciones
app.get('/api/materias', async(req, res) => {
    try {
        const connection = db.promise();

        // Consulta 1: Datos básicos de materias
        const [materias] = await connection.query('SELECT codigo, descripcion, numobj, minaprueba FROM materia ORDER BY descripcion ASC');

        // Consulta 2: Datos de objetivos
        ...
        // Consulta 3: Datos de calificaciones
        ...
        // Imprimir error real en la terminal del servidor para diagnóstico
        console.error('✅ ERROR CRÍTICO EN /api/materias (Este es el error que buscamos):', err);

        // --- CORRECCIÓN AQUÍ ---
        // NO usar res.redirect('/')
        // Enviar error 500 al navegador para que el frontend maneje el aviso
        res.status(500).json({
            success: false,
            message: 'Error grave en el servidor al consultar materias. Verifique la consola del servidor.'
        });
    }
});

// ... (demás rutas de materias permanecen igual ...
...
app.get('/api/materias-objetivos/:codigoMateria', async(req, res) => {
    ...
    try {
        const connection = db.promise();
        const [materiaRows] = await connection.query('SELECT id, codigo, descripcion, numobj, minaprueba FROM materia WHERE codigo = ?', [codigoMateria]);
...
// ==========================================
// RUTAS PARA EL MÓDULO DE CONTROL ASESORÍAS
// ==========================================
...
// ==========================================
// RUTA PARA OBTENER LOS TIPOS DE ASESORÍA (Para llenar el SELECT)
// ==========================================
app.get('/tipo_asesoria', (req, res) => {
    res.sendFile(path.join(__dirname, 'views', 'tipo_asesoria.html'));
});
app.get('/api/tipo_asesoria', (req, res) => {
    const query = 'SELECT id,codigo_ase, descripcion_asesoria FROM tipoasesoria ORDER BY descripcion_asesoria DESC';

    db.query(query, (err, results) => {
        if (err) {
            console.error('Error al obtener tipos de asesoría:', err);
            return res.status(500).json({ success: false, message: 'Error en el servidor al consultar tipos de asesoría' });
        }
